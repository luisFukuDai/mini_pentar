...
void  INT_Excep_MTU2_TGIA2(void){
  PORTD.DR.BIT.B1 = 1 - PORTD.DR.BIT.B1;
}
//;0x01F8  MTU2_TGIB2
...
//  wheelControl();
  ...
  if(newData){
    newData = 0;
    wheelMotor[0].dQD = ((float)r0M[0])/2048.0*4.0;
    wheelMotor[1].dQD = ((float)r0M[1])/2048.0*4.0;
  }
#else
  if(newData){
    newData = 0;
    wheelMotor[0].dQD = ((float)r1M[0])/2048.0*4.0;
    wheelMotor[1].dQD = ((float)r1M[1])/2048.0*4.0;
  }
#endif
...
{
  //rxSci0 = SCI0.RDR;
  PORTD.DR.BIT.B0 = 1 - PORTD.DR.BIT.B0;

  short data;

  data = SCI0.RDR;
  enqueueRx0((char)data);
}
...
void INT_Excep_SCI0_TXI0(void)
{
  ...
  PORTD.DR.BIT.B2 = 1 - PORTD.DR.BIT.B2;

  if (SCI0.SSR.BIT.TDRE == 1) {
    if( (data = dequeueTx0()) != -1) {
      SCI0.TDR = (char) data;
      //      PORTD.DR.BIT.B3 = 1 - PORTD.DR.BIT.B3;
    }
  }
  //  else {
  //    PORTD.DR.BIT.B2 = 1 - PORTD.DR.BIT.B2;
